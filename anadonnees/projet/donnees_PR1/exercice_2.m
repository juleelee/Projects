...
f=-ones(length(X),1);
Hf = diag(Y')*(X*X')*diag(Y);
Aeq=Y';
beq=0;
lb = zeros(length(X),1);
[alpha,~,code_retour] = quadprog(Hf,f,[],[],Aeq,beq,lb,[]);

%On calcules les alphai puis W = somme des alpha_i y_i x_i
%%Puis on applique l'appartenance d'un vecteur support pour trouver c


w = X'*(alpha.*Y);

comp = abs(alpha)>=10e-6;
X_VS = X(comp,:);
Y_VS  = Y(comp,:);
alphac = alpha(comp);
xi = X_VS(1,:);
