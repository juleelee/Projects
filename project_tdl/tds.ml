...
type tds =
  ...
  | Courante of tds * (string,info_ast) Hashtbl.t * (string, info_ast) Hashtbl.t
...
let creerTDSMere () = Courante (Nulle, Hashtbl.create 100,Hashtbl.create 50)

(* Création d'une table des symboles fille *)
(* Le paramètre est la table mère *)
let creerTDSFille mere = Courante (mere, Hashtbl.create 100, Hashtbl.create 50)
...
let ajouter tds nom info =
  ...
  | Courante (_, c, _) -> Hashtbl.add c nom info 


(* Ajoute une information de label dans la table des symboles locale *)
(* tds : la tds courante *)
(* string : le nom du label *)
(* info : l'information à associer au label *)
(* Si le label est déjà présent dans TDS, l'information est écrasée *)
(* retour : unit *)
let ajouterLabel tds nom info =
  match tds with
  | Nulle -> failwith "Ajout de label dans une table vide"
  | Courante (_, _, c_label) -> Hashtbl.add c_label nom info
...
let chercherLocalement tds nom =
  match tds with
  | Nulle -> None
  | Courante (_,c,_) ->  find_opt c nom 

(* Recherche les informations d'un label dans la tds locale et parente *)
(* tds : la tds courante *)
(* string : le nom du label *)
(* retour : info option *)
let rec chercherLabel tds nom =
  match tds with
  | Nulle -> None
  | Courante (m, _, c_label) ->
      match find_opt c_label nom with
      | Some _ as i -> i
      | None -> chercherLabel m nom
...
let string_of_info info =
  ...
  | InfoFun (n, t, tp) -> "Fonction " ^ n ^ " : " ^ (List.fold_right (fun elt tq -> if tq = "" then (string_of_type elt) else (string_of_type elt) ^ " * " ^ tq) tp "") ^ " -> " ^ (string_of_type t)
  | InfoLabel n -> "Label " ^ n
...
let afficher_locale tds =
  match tds with
  | Nulle -> print_newline ()
  |Courante (_,c,_) -> Hashtbl.iter ( fun n info -> (print_string (n^" : "^(string_of_info (info_ast_to_info info))^"\n"))) c
...
let afficher_globale tds =
  let rec afficher tds indent =
    match tds with
    | Nulle -> print_newline ()
    | Courante (m,c,_) -> if Hashtbl.length c = 0
