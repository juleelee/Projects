...
struct
  ...
  let rec string_of_affectable a =
    ...
    | AccesTab (af, exp) -> (string_of_affectable af) ^ "[" ^ (string_of_expression exp) ^ "]"
  ...
  and string_of_expression e =
    ...
    | Adresse n -> "&"^n^" "
    | CreateTab (t,exp) -> "new " ^ (string_of_type t) ^  " [" ^ (string_of_expression exp) ^ "] "
    | InitTab le -> "{" ^ (print_tab (List.map string_of_expression le)) ^ "}"
  ...
  let rec string_of_instruction i =
    ...
    (* Boucle For*)
    | For (n1,e1,e2,n2,e3,b) -> "For : FOR ( int "^n1^" = "^(string_of_expression e1)^"; "^(string_of_expression e2)^"; "^n2^" = "^(string_of_expression e3)^") "^((List.fold_right (fun i tq -> (string_of_instruction i)^tq) b ""))
    (* Affectations *)
    | Affectation (af,e) ->  "Affectation  : "^(string_of_affectable af)^" = "^(string_of_expression e)^"\n"
    | CallGoto (n) -> "Appel Goto : " ^n^"\n"
    | DefGoto (n) -> "Definition de l'étiquette goto : " ^n^"\n"
